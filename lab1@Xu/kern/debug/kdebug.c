...
print_stackframe(void) {
      ...
      */
   //函数内发生函数调用，打印print_stackframe函数的调用栈信息；
    //打印的是上一个函数的栈帧——EBP往上，
    uint32_t ebp, eip;
    //EBP：栈底
    ebp=read_ebp();
   // 调用read_eip函数来获取指令的位置--EIP存的是下一条的
    eip=read_eip();
    int i, j;
    //ebp最后为哪样会为0？？？是系统设置的（参见练习3结尾）
    for (i = 0; ebp != 0 && i < STACKFRAME_DEPTH; i++) {//这里面有好几个函数的栈帧，是嵌套调用的
        //打印 ebp（上一层栈底）, eip（返回地址）
        cprintf("ebp:0x%08x eip:0x%08x args:", ebp, eip);
       
        //栈从高地址往低地址增长
        //实验假设函数有4个参数
       //+2表示ebp往下移动两格！（跳过返回地址）——得到数组的首地址
        ...
        //看看自己当前在哪个位置——看打印的是哪个函数的栈帧
        print_debuginfo(eip - 1);//EIP存的是下一条的地址，所以-1表示当前地址
        //EBP+1是返回地址，也就是上一个函数的下一条指令，也就是EIP的值
